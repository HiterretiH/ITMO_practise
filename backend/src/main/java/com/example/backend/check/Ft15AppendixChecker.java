...
/**
 * оформление приложений (п. 4.11.2).
 * <p>
        ...
        for (int k = 0; k < appendixHeadingIndices.size(); k++) {
                ...
                for (int j = 0; j < idx; j++) {
                    ...
                    if (prev.getPageIndex() != null
                        ...
                        add(issues,
                                String.format(
                                        Locale.ROOT,
                                        "%s — заголовок «%s» (абзац на стр. %d) должен быть наверху страницы: "
            ...
            if (al == null || !"CENTER".equalsIgnoreCase(al)) {
                add(issues,
                        String.format(
                                Locale.ROOT,
                                "%s — заголовок «%s» (абзац на стр. %s) должен быть выровнен по центру страницы "
            ...
            if (!isAppendixHeadingAllUppercase(p.getText())) {
                add(issues,
                        String.format(
                                Locale.ROOT,
                                "%s — заголовок «%s» (абзац на стр. %s) должен быть набран прописными (заглавными) буквами.",
                                REQ,
                                label,
                                formatIssuePage(page)));
            }
            ...
            if (k >= APPENDIX_LETTER_ORDER.length()) {
                add(issues,
                        String.format(
                                Locale.ROOT,
                                "%s — заголовок «%s» (абзац на стр. %s): слишком много приложений подряд "
            ...
            if (letterChar == null) {
                add(issues,
                        String.format(
                                Locale.ROOT,
                                "%s — в заголовке «%s» (абзац на стр. %s) не удалось определить букву приложения.",
                                REQ,
                                label,
                                formatIssuePage(page)));
                continue;
            ...
            if (letterOrdinal < 0) {
                add(issues,
                        String.format(
                                Locale.ROOT,
                                "%s — в заголовке «%s» (абзац на стр. %s) недопустимая буква приложения «%s» "
            ...
            if (letterOrdinal != k) {
                ...
                add(issues,
                        String.format(
                                Locale.ROOT,
                                "%s — заголовок «%s» (абзац на стр. %s): приложения должны идти по порядку букв подряд (А, Б, В, …); "
